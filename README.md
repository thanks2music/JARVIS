...
claude mcp add --transport stdio [--scope <scope>] [-e KEY=VALUE]... <name> -- <command> [args...]
...
**変換の公式**:
```
claude mcp add --transport stdio [-s <scope>] [-e KEY=VALUE]... <name> -- <command> <args...>
```
...
claude mcp add --transport stdio -s user -e BRAVE_API_KEY=YOUR_API_KEY_HERE brave-search -- npx -y @brave/brave-search-mcp-server

# local スコープ（デフォルト、現在のプロジェクトのみ）
claude mcp add --transport stdio -e BRAVE_API_KEY=YOUR_API_KEY_HERE brave-search -- npx -y @brave/brave-search-mcp-server
```
...
> ⚠️ **警告**: 任意コマンドの実行許可はデータ損失・システム破損・プロンプトインジェクションによるデータ流出のリスクがある。`--dangerously-skip-permissions` はインターネット接続なしのサンドボックス環境でのみ使用すること。
...
> ⚠️ **注意**: チェックポイントはClaudeが行った変更のみを追跡する。外部プロセスの変更は対象外。gitの代替ではない。
...
> ⚠️ **警告**: 任意コマンドの実行はデータ損失・システム破損・プロンプトインジェクション攻撃によるデータ流出のリスクがある。`/sandbox` を有効化するとチェックをバイパスする代わりに事前に境界を定義するため、同様の自律性をより高いセキュリティで得られる。
